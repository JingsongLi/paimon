    ...
    fn seek(&mut self, pos: u64) -> io::Result<()> {
        Python::with_gil(|py| {
            self.file
                ...
                .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("seek: {}", e)))?;
            Ok(())
        })
    }
    ...
    fn read_exact(&mut self, buf: &mut [u8]) -> io::Result<()> {
        Python::with_gil(|py| {
            ...
            let bytes: &Bound<PyBytes> = result
                .downcast_bound(py)
                .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("downcast: {}", e)))?;
...
impl IVFPQReader {
    ...
    fn search<'py>(
        ...
        let id_array = PyArray1::from_vec(py, ids);
        let dist_array = PyArray1::from_vec(py, dists);
