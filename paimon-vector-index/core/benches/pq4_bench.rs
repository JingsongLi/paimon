...
use paimon_vindex_core::fastscan::{pack_codes_block_layout, fastscan_4bit};
...
fn main() {
    ...
    println!("Dataset: {}K vectors, d={}, nlist={}, nprobe={}, k={}", n/1000, d, nlist, nprobe, k);
    println!();
    ...
    for v in centers.iter_mut() { *v = next() * 50.0; }
    let mut data = vec![0.0f32; n * d];
    for i in 0..n {
        let c = i % num_clusters;
        for j in 0..d { data[i * d + j] = centers[c * d + j] + next(); }
    }
    ...
    let start = Instant::now();
    for _ in 0..5 { idx8.search(queries, nq, k, nprobe, &mut d8, &mut l8); }
    ...
    let start = Instant::now();
    for _ in 0..5 { idx4.search(queries, nq, k, nprobe, &mut d4, &mut l4); }
    ...
    let biggest_list = idx4.codes.iter().enumerate()
        .max_by_key(|(_, c)| c.len()).map(|(i, _)| i).unwrap();
    ...
    idx4.pq.compute_distance_table(&residual, MetricType::L2, &mut sim_table);
    ...
    for qi in 0..nq_r {
        ...
        let mut bf: Vec<(f32, i64)> = (0..n).map(|i| {
            let mut dist = 0.0f32;
            for j in 0..d { let diff = query[j] - data[i*d+j]; dist += diff*diff; }
            (dist, i as i64)
        }).collect();
        bf.sort_by(|a, b| a.0.partial_cmp(&b.0).unwrap());
        let gt: HashSet<i64> = bf[..k].iter().map(|&(_, id)| id).collect();
        let base = qi * k;
        recall_4 += l4[base..base+k].iter().filter(|id| gt.contains(id)).count();
        recall_8 += l8[base..base+k].iter().filter(|id| gt.contains(id)).count();
    }
    ...
    println!("╠══════════════════════════════════════════════════════════════════╣");
    println!("║                    8-bit (M={})       4-bit (M={})              ║", m8, m4);
    println!("╠══════════════════════════════════════════════════════════════════╣");
    println!("║ Storage/vec:       {} bytes           {} bytes                  ║", codes_8/n, codes_4/n);
    println!("║ Build time:        {:.2}s              {:.2}s                   ║", build8.as_secs_f64(), build4.as_secs_f64());
    println!("║ Query (nq={}):    {:.1}ms ({:.0}μs/q)    {:.1}ms ({:.0}μs/q)   ║",
        nq, q8*1000.0, q8*1e6/nq as f64, q4*1000.0, q4*1e6/nq as f64);
    println!("║ Recall@{}:         {:.1}%              {:.1}%                   ║",
        k, recall_8 as f64 / (nq_r*k) as f64 * 100.0, recall_4 as f64 / (nq_r*k) as f64 * 100.0);
    println!("║ FastScan (1 list): -                  {:.0}μs ({} vecs)         ║", fs_us, list_n);
    println!("╚══════════════════════════════════════════════════════════════════╝");
    ...
    println!("╠══════════════════════════════════════════════════════════════════╣");
    println!("║ 8-bit query:       {:.0}μs/q     ~8μs/q      ~15μs/q             ║", q8*1e6/nq as f64);
    println!("║ 4-bit query:       {:.0}μs/q     ~3μs/q      ~5μs/q              ║", q4*1e6/nq as f64);
    println!("║ 4-bit fastscan:    {:.0}μs/list  ~1μs/list   ~2μs/list           ║", fs_us);
    println!("║ Build (100K):      {:.1}s        ~2.5s       ~3s                 ║", build8.as_secs_f64());
    println!("║ 8-bit Recall@10:   {:.0}%         ~45%        ~40%               ║", recall_8 as f64 / (nq_r*k) as f64 * 100.0);
    println!("║ 4-bit Recall@10:   {:.0}%         ~35%        ~30%               ║", recall_4 as f64 / (nq_r*k) as f64 * 100.0);
    println!("╠══════════════════════════════════════════════════════════════════╣");
