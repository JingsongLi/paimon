...
impl DiskShuffler {
    ...
    pub fn write_vector(&mut self, partition_id: u32, row_id: i64, vector: &[f32]) -> io::Result<()> {
    ...
    pub fn read_partition(&self, partition_id: u32) -> io::Result<(Vec<i64>, Vec<f32>)> {
        ...
        for _ in 0..self.count {
            ...
            let pid = u32::from_le_bytes([record_buf[0], record_buf[1], record_buf[2], record_buf[3]]);
            if pid == partition_id {
                let row_id = i64::from_le_bytes([
                    record_buf[4], record_buf[5], record_buf[6], record_buf[7],
                    record_buf[8], record_buf[9], record_buf[10], record_buf[11],
                ]);
                ids.push(row_id);
                for i in 0..self.dim {
                    let off = RECORD_OVERHEAD + i * 4;
                    let v = f32::from_le_bytes([
                        record_buf[off], record_buf[off + 1],
                        record_buf[off + 2], record_buf[off + 3],
                    ]);
                    vectors.push(v);
                }
            }
    ...
    pub fn read_all_partitions(&self) -> io::Result<(Vec<Vec<i64>>, Vec<Vec<f32>>)> {
        ...
        for _ in 0..self.count {
            reader.read_exact(&mut record_buf)?;
            let pid = u32::from_le_bytes([record_buf[0], record_buf[1], record_buf[2], record_buf[3]]) as usize;
            let row_id = i64::from_le_bytes([
                record_buf[4], record_buf[5], record_buf[6], record_buf[7],
                record_buf[8], record_buf[9], record_buf[10], record_buf[11],
            ]);
...
mod tests {
    ...
    fn test_shuffler_roundtrip() {
        ...
        shuffler.write_vector(0, 100, &[1.0, 2.0, 3.0, 4.0]).unwrap();
        shuffler.write_vector(1, 200, &[5.0, 6.0, 7.0, 8.0]).unwrap();
        shuffler.write_vector(0, 300, &[9.0, 10.0, 11.0, 12.0]).unwrap();
        shuffler.write_vector(2, 400, &[13.0, 14.0, 15.0, 16.0]).unwrap();
