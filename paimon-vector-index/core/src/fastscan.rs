...
pub fn fastscan_4bit(
    ...
    let num_blocks = (n + BBS - 1) / BBS;
    let start_block = flat_end / BBS + (if flat_end % BBS > 0 { 1 } else { 0 });
        ...
        if is_x86_feature_detected!("avx2") {
            unsafe {
                fastscan_blocks_avx2(
                    &qtable, codes, cs, start_block, num_blocks, block_size, qmin, range, m, dists,
                );
            }
...
mod tests {
    ...
    fn test_fastscan_correctness() {
        ...
        // Random-ish codes
        let codes_row: Vec<u8> = (0..n * cs)
            .map(|i| ((i * 7 + 3) % 256) as u8)
            .collect();

        // Random-ish distance table [M * 16]
        let sim_table: Vec<f32> = (0..m * 16)
    ...
    fn test_fastscan_large() {
        ...
        for i in 0..n {
            assert!(result[i] > 0.0, "Distance at {} is non-positive: {}", i, result[i]);
            assert!(result[i] < 10000.0, "Distance at {} seems too large: {}", i, result[i]);
        }
    }
}
